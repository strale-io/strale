...
export const SETTLED_MATTERS: Array<{ id: string; re: RegExp; settledBy: string }> = [
  {
    ...
    re: /\b(?:stands?|standing) (?:or|and) (?:it |be |being |is |get |gets )?rever(?:s|t)|(?:revert(?:ing|ed)?|revers(?:e|ing|ed|al)|re-?open(?:ing|ed)?|un-?clos(?:e|ing|ed)|re-?run|back(?:ing)? out|claw .{0,12}back|claw(?:ing)? back|walk(?:ing)? .{0,12}back|reinstat(?:e|ing|ed)|restor(?:e|ing|ed)|un-?do(?:ing)?|undone|roll(?:ing)? back|put .{0,25}back|set .{0,25}back|withdraw(?:ing|n)?|approve or reject)[^.]{0,45}\b(?:eleven|11)\b[^.]{0,25}(?:rows?|records?|transactions?|charges?|closures?)|\b(?:eleven|11)\b[^.]{0,25}(?:rows?|records?|transactions?|charges?|closures?)[^.]{0,45}(?:revert(?:ing|ed)?|revers(?:e|ing|ed|al)|re-?open(?:ing|ed)?|un-?clos(?:e|ing|ed)|re-?run|back(?:ing)? out|claw .{0,12}back|claw(?:ing)? back|walk(?:ing)? .{0,12}back|reinstat(?:e|ing|ed)|restor(?:e|ing|ed)|un-?do(?:ing)?|undone|roll(?:ing)? back|put .{0,25}back|set .{0,25}back|withdraw(?:ing|n)?|approve or reject)|\b(?:eleven|11)\b[^.]{0,25}(?:rows?|records?|transactions?|charges?|closures?)[^.]{0,30}(?:should|to) stand|(?:revert(?:ing|ed)?|revers(?:e|ing|ed|al)|re-?open(?:ing|ed)?|un-?clos(?:e|ing|ed)|re-?run|back(?:ing)? out|claw .{0,12}back|claw(?:ing)? back|walk(?:ing)? .{0,12}back|reinstat(?:e|ing|ed)|restor(?:e|ing|ed)|un-?do(?:ing)?|undone|roll(?:ing)? back|put .{0,25}back|set .{0,25}back|withdraw(?:ing|n)?|approve or reject)[^.]{0,45}(?:this morning|07:50|the reconciliation|that reconciliation)|(?:this morning|07:50|the reconciliation|that reconciliation)[^.]{0,45}(?:revert(?:ing|ed)?|revers(?:e|ing|ed|al)|re-?open(?:ing|ed)?|un-?clos(?:e|ing|ed)|re-?run|back(?:ing)? out|claw .{0,12}back|claw(?:ing)? back|walk(?:ing)? .{0,12}back|reinstat(?:e|ing|ed)|restor(?:e|ing|ed)|un-?do(?:ing)?|undone|roll(?:ing)? back|put .{0,25}back|set .{0,25}back|withdraw(?:ing|n)?|approve or reject)|\b(?:eleven|11)\b[^.]{0,20}(?:unfinished|stranded|executing|unclosed)|\b(?:eleven|11)-row reconciliation|\bmanual_reconciliation\b|stranded (?:executing )?(?:rows?|records?|transactions?|charges?)/i,
    settledBy:
