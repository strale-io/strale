...
x402GatewayV2.on(["GET", "POST"], ["/solutions/:slug", "/v2/solutions/:slug"], async (c) => {
  ...
  try {
    ...
    // answer 413 — and this catch turned it straight back into a 400, which is
    // where the first version of this fix quietly failed. Hono's bodyLimit
...
x402GatewayV2.on(["GET", "POST"], ["/:slug", "/v2/:slug"], async (c) => {
  ...
  try {
    ...
    // See the identical guard on the solutions path above.
    if ((err as { name?: string })?.name === "BodyLimitError") throw err;
    return c.json({ error: "Invalid request body. Expected JSON." }, 400);
  }
