...
// Firmenbuchnummer: up to 6 digits + one check letter, e.g. "FN 93363 z".
const FNR_RE = /^(\d{1,6})\s*([a-z])$/;
...
async function soapCall(bodyXml: string): Promise<string> {
  ...
  // the human-readable Reason rather than the raw envelope.
  const fault = text.match(
    /<(?:\w+:)?Fault[\s>][\s\S]*?<(?:\w+:)?Text[^>]*>([\s\S]*?)<\/(?:\w+:)?Text>/,
  );
  if (fault) {
    throw new Error(`Firmenbuch API fault: ${fault[1].trim()}`);
  }
  if (!r.ok) {
...
/** Strip SOAP namespace prefixes (ns6: etc.) so parsing is prefix-agnostic. */
function stripNs(xml: string): string {
  return xml.replace(/\b[A-Za-z][A-Za-z0-9]*:(?=[A-Z_])/g, "");
}
...
async function resolveByName(name: string): Promise<SearchHit> {
  ...
  if (hits.length === 0) {
    throw new Error(
      `No Austrian company found in the Firmenbuch for '${name}'. Provide the Firmenbuchnummer (e.g. "FN 93363 z") if you have it.`,
    );
  }
  if (hits.length === 1) return hits[0];
  // Registry name searches never rank by relevance — an unqualified pick of
  // hits[0] returns the wrong company. Accept a unique case-insensitive
  // exact match; otherwise refuse with the candidate list.
  const lowered = name.trim().toLowerCase();
  const exact = hits.filter((h) => h.name.trim().toLowerCase() === lowered);
  if (exact.length === 1) return exact[0];
  const preview = hits
    .slice(0, 5)
    .map((h) => `${h.name} (FN ${h.fnr}${h.seat ? `, ${h.seat}` : ""})`)
    .join("; ");
  throw new Error(
    `Ambiguous company name '${name}': ${hits.length} Firmenbuch matches. Call again with 'registration_number'. Candidates: ${preview}`,
  );
}
...
export function parseAuszug(xml: string) {
  ...
  const currentBlock = (tag: string): string | null => {
    // A DKZ block with AUFRECHT="true" is the currently valid entry;
    // historical entries carry AUFRECHT="false".
  ...
  const nameBlock = currentBlock("FI_DKZ02");
  const addrBlock = currentBlock("FI_DKZ03");
  const seatBlock = currentBlock("FI_DKZ06");
  const formBlock = currentBlock("FI_DKZ07");
  const courtBlock = currentBlock("FI_DKZ09");
  ...
  for (const pm of plain.matchAll(/<PER([^>]*)>([\s\S]*?)<\/PER>/g)) {
    ...
    if (!pnr) continue;
    const block = pm[2].match(/<PE_DKZ02([^>]*)>([\s\S]*?)<\/PE_DKZ02>/);
    if (!block || !block[1].includes('AUFRECHT="true"')) continue;
    const formatted = grabIn(block[2], "NAME_FORMATIERT");
    const bare = [grabIn(block[2], "VORNAME"), grabIn(block[2], "NACHNAME")]
      .filter(Boolean)
      .join(" ");
    persons.set(pnr, {
      name: formatted ?? (bare || null),
      birth: grabIn(block[2], "GEBURTSDATUM"),
    });
  }
  ...
  for (const fm of plain.matchAll(/<FUN([^>]*)>([\s\S]*?)<\/FUN>/g)) {
    ...
    const dkz = fm[2].match(/<FU_DKZ10([^>]*)>([\s\S]*?)<\/FU_DKZ10>/);
    if (!dkz || !dkz[1].includes('AUFRECHT="true"')) continue; // former officer
    const person = pnr ? persons.get(pnr) : undefined;
    if (!person?.name) continue;
    const reprText = [...dkz[2].matchAll(/<TEXT>([^<]*)<\/TEXT>/g)]
      .map((t) => t[1].trim())
      .join(" ");
    representatives.push({
      ...
      start_date: toIsoDate(grabIn(dkz[2], "DATVON")),
  ...
  return {
    ...
    historical_registration_number: courtBlock
      ...
      : null,
    homepage: grabIn(plain, "HOMEPAGE"),
    euid: grabIn(plain, "EUID"),
    representatives,
    source_as_of: attr("ABFRAGEZEITPUNKT"),
    source_checksum: attr("PRUEFSUMME"),
    // The Kurzinformation extract of a struck-off company still resolves;
    // deletion shows up as no AUFRECHT="true" name/seat blocks remaining.
    is_current: Boolean(nameBlock),
  };
}
...
registerCapability("austrian-company-data", async (input: CapabilityInput) => {
  const rawFnr =
    (input.registration_number as string) ??
    (input.fnr as string) ??
    (input.identifier as string) ??
    "";
  const rawName = (input.company_name as string) ?? "";
  ...
  if (typeof rawFnr === "string" && rawFnr.trim()) {
  ...
  } else if (typeof rawName === "string" && rawName.trim().length >= 2) {
  ...
  // The search STATUS field carries insolvency/liquidation markers; the
  // Kurzinformation extract itself has no dedicated status element.
  const statusNote = searchStatus || null;
  const status = parsed.is_current ? "active" : "inactive";
  ...
  return {
    output: {
      ...
      // Legacy convention (NO/CZ/EE/SK): natural persons only, name-only.
      directors: parsed.representatives
...
export { FNR_RE, normaliseFnr, parseSearchResults, stripNs };
