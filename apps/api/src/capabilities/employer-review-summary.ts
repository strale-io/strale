...
registerCapability("employer-review-summary", async (input: CapabilityInput) => {
  ...
  const output = await extractJsonWithLlm({
    ...
    maxTokens: 2000,
    prompt: `Extract employer review information for "${companyName}" from this page.
...
Page text:
${text.slice(0, 12000)}`,
    truncationGuidance: "This employer's review page produced more content than fits in one call.",
    parseFailureError: () => new Error("Failed to extract review data from the page."),
  });
