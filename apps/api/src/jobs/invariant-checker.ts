...
export async function runInvariantChecks(): Promise<void> {
  ...
  // Checks 1, 2, 4 retired with the SQS engine (DEC-20260503-B): score
  // persistence drift, solution score sanity, and freshness decay drift
  // all keyed on the dual-profile/matrix substrate. Check 3 (orphaned
  // solution steps) is retained but its matrix_sqs criterion is removed.
  ...
  // that was retired with the SQS engine; the function definition went
  // with it). Re-implement as part of the source_health build-out (PR2+).
...
async function checkOrphanedSolutionSteps(): Promise<{ alerts: number; checked: number }> {
  ...
  // Find solution steps where the capability is missing or inactive.
  // The matrix_sqs criterion was removed with the SQS engine (DEC-20260503-B).
