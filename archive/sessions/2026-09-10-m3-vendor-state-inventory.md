...
| S41 | `rg -n "generate\.js" .` (repo-wide literal search, for R12's reach) | 3 non-report hits: `packages/strale-capabilities/package.json:8` (`"generate": "node generate.js"`, the package's own npm script), `packages/strale-capabilities/generate.js:6` (its own usage comment), and unrelated `*-generate.js` capability-executor imports in `apps/api/src/capabilities/*.test.ts` (different files, matched only because they share the `-generate.js` suffix). No `.github/workflows/*.yml` references `generate.js` |
