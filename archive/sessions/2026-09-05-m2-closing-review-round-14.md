...
Round 14 of the M2 closing independent review, run at commit
`d0e21ecdb3009c8ce83a5345c95755c8cc386ec1` (the final round after rounds 1
through 13, archived at `archive/sessions/2026-09-05-m2-closing-review-round-1.md`
through `-round-13.md`, each of whose confirmed findings is withdrawn by
`docs/decisions/records/DEC-20260905-B.md` through `-O.md`). Six fresh,
read-only reviewers, none the author of any reviewed content, partitioned
the 245 formal records under `docs/decisions/records/` (P1 through P4: 41
records each; P5: 34 records; P6: 47 records), applied the quotation
convention `DEC-20260905-C` through `-O` state, ran the operator checker
(`scripts/m2-quote-fidelity.mjs`) against the parsed Notion export and the
sibling `strale-frontend` checkout at `--min-chars 12`, alongside each
partition's own read of every quotation, evidence path, relation, and at
least ten "status on" code claims against the reviewed commit, and a full
gate run.

All six partitions returned `PARTITION VERDICT: PASS` with no numbered
findings. All nine gates exited 0. Partition P5 separately flagged, as an
"ancillary observation, not a finding against a graded record," that
`archive/sessions/2026-09-05-decision-collision-resolution-DEC-20260420-D.md`
(a resolution report outside P5's own 34 assigned record files) repeats a
statement about `PII_CATEGORY_ENUM` that `DEC-20260905-C` item 34 already
withdraws from the formal record the statement belongs to, without any
`DEC-20260905-*` record naming the resolution report itself. The
consolidator that reviewed this round's six reports ruled the statement a
stated limitation and wrote a PASS verdict on a pull request; the
independent review of that verdict pull request ruled that an unlisted
stale resolution-report statement is a finding under the round's rule
(a), since that rule's exemption covers only a stale resolution-report
statement an amending record lists, and no record listed this one. The
orchestrator accepted that ruling and closed the verdict pull request
(#611) unmerged. The round's verdict is therefore FAIL on that one item.
...
All six partitions passed and all nine gates exited 0; the consolidated
verdict is FAIL on the one item below.

1. **`archive/sessions/2026-09-05-decision-collision-resolution-DEC-20260420-D.md`,
   lines 91-92.** Found by partition P5 (as an ancillary observation
   outside its own graded record set) and ruled a finding by the
   independent review of the round's verdict pull request. The report
   states, in its own voice as current fact: "`apps/api/src/lib/onboarding-gates.ts`
   still enforces `PII_CATEGORY_ENUM` exactly as this row specifies."
   That statement is false at this commit: `apps/api/src/lib/onboarding-gates.ts:242-260`
   defines `PII_CATEGORY_ENUM` with 14 entries, the 12 the underlying row
   named plus `nationality` and `political_affiliation`, added per an
   inline comment dated 2026-04-30. `DEC-20260905-C` item 34 already
   withdraws this exact statement from the formal record it belongs to,
   `DEC-20260420-D--notion-34867c87082c81f0827eedf29d133600`, but names
   only that record, never this resolution report; the round's rule (a)
   exempts only a stale resolution-report statement an amending record
   lists, so this unlisted one stood as a live finding.

This item is corrected by `DEC-20260905-P`
(`docs/decisions/records/DEC-20260905-P.md`), which notes the resolution
report by name without editing it, the same treatment `DEC-20260905-G`
already gives two other resolution reports repeating a withdrawn
statement.
