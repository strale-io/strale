...
Status: canonical approved website foundation  
Visual catalogue: `design-system.html`  
Current implementation: `/homepage-v2`  
Shared tokens: `assets/design-tokens.css`
Adaptive layout contract: `ADAPTIVE-LAYOUT.md`

Canonical repository location:
`public/strale-site/design-system/`

This package supersedes the earlier Luminous Utility/Luminous Spectrum
explorations under `docs/website-redesign/foundations/` as implementation
authority for the marketing website. Those files remain historical evidence.
Future changes are made here first and then propagated to consumers.

This file governs the Strale marketing website. It does not replace the
separate product-application UI system.
...
Dark process frames use purpose-built dark raster plates. Use the named token
directly and do not apply a component-level blend mode; the artwork already
contains the approved dark atmosphere, fold geometry, grain, and tonal range.

| Token | Asset | Primary role |
|---|---|---|
| `--pattern-folded-dark-midnight` | `assets/pattern-folded-dark-midnight-v2.png` | Neutral dark product framing. |
| `--pattern-folded-dark-cobalt` | `assets/pattern-folded-dark-cobalt-v2.png` | Infrastructure, routing, and technical proof. |
| `--pattern-folded-dark-dusk` | `assets/pattern-folded-dark-dusk-v2.png` | Orchestration and closing transitions. |
| `--pattern-folded-dark-mineral` | `assets/pattern-folded-dark-mineral-v2.png` | Trust, validation, and reliability. |
| `--pattern-folded-dark-spectrum` | `assets/pattern-folded-dark-spectrum-v2.png` | Major multi-signal product proof. |
| `--pattern-folded-dark-ember` | `assets/pattern-folded-dark-ember-v2.png` | Action, urgency, and customer outcomes. |

Dark glass product proof uses `--surface-glass-inverse`,
`--border-glass-inverse`, and `--shadow-glass-inverse`. Keep those tokens as a
set; do not synthesize a local dark-glass finish inside a component.

Use the `-raised` inverse-glass set only for the single dominant result surface
inside a dark process frame: `--surface-glass-inverse-raised`,
`--border-glass-inverse-raised`, and `--shadow-glass-inverse-raised`. It adds
edge light and deeper elevation without turning every nested row into a card.
...
7. Add a new variant only when a recurring narrative role cannot be served by
   the approved plate family. Every addition requires a named asset, a token, a
   usage note, and a responsive specimen in `design-system.html`.
8. Dark process plates belong inside explanatory process frames. They never replace
   the hero sweep and must retain a separate, legible product-proof surface.
...
The gradients remain CSS tokens for code-native frames. Dark folded process
plates are the reviewed exception: their restrained semantic hue is integrated
into the raster so translucent overlap and light remain physically coherent.
Do not layer a second semantic gradient over those dark plates. Light Quiet
Material rasters continue to supply form and copy-safe space independently from
their code-native semantic frame.
...
- The six named dark folded v2 plates are approved for marketing process
  frames. Use only their named tokens, keep product UI in live HTML/CSS, and
  verify the chosen crop at the target breakpoint.
