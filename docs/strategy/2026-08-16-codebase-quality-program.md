...
| # | Target | Verified by |
|---|--------|-------------|
| T2.1 | **CLOSED 2026-08-18.** Backend half (landed 08-17): `npm run typecheck` covers `apps/api/src`, `apps/api/scripts`, `apps/api/test` and all four published TS packages. Dashboard half: **the gap was real and was fixed in strale-frontend, not here.** Investigation (empirical, not config-reading): strale-frontend's CI runs test → build → lint:ratchet, all green — and *none* type-checks. `vite build` uses SWC (strips types without checking), vitest runs untyped, the ESLint ratchet is not type-aware, Cloudflare Pages runs the same untypechecked build, and no `typecheck` script existed. Injecting `const x: number = "str"` fails `tsc -b` and passes `vite build` on the same tree. Fix shipped as strale-io/strale-frontend#19 (adds `typecheck: tsc -b`, runs it in that repo's CI before Test/Build, gitignores the artifacts). **A PR gate in THIS repo was built and then deliberately removed**: strale-frontend depends on no backend package, so a backend change cannot break its compilation — gating backend PRs on it would only ever surface pre-existing frontend errors and blame unrelated PRs. What remains here is one weekly-drift backstep whose sole job is to notice if the frontend's own gate stops working (the hollow-gate failure mode this program hit three times). | strale-frontend#19's own CI run shows the Typecheck step executing and passing; fail-before proven by the injected-error probe. Weekly backstop verified by the same checkout that the AuditRecord contract already uses. |
