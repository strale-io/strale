...
| # | Target | Verified by |
|---|--------|-------------|
| T2.1 | **CLOSED 2026-08-18.** Backend: `npm run typecheck` covers `apps/api/src`, `apps/api/scripts`, `apps/api/test` and all four published TS packages. Dashboard: the gap was real and was fixed in the repo that had it. strale-frontend's CI ran test → build → lint, all green, and *none* type-checked — `vite build` uses SWC (strips types without checking), vitest ran untyped, the ESLint ratchet is not type-aware, Cloudflare Pages runs the same build, and no `typecheck` script existed. Injecting `const x: number = "str"` fails `tsc -b` and passes `vite build` on the same tree. Fixed by strale-io/strale-frontend#19 (**merged**) — adds `typecheck: tsc -b` and runs it in that repo's CI ahead of Test/Build. A PR gate in THIS repo was built and deliberately removed: strale-frontend imports no backend package, so a backend change cannot break its compilation, and gating backend PRs on it would only surface pre-existing frontend errors against unrelated PRs. What remains here is a weekly-drift backstop that watches whether the frontend's own gate still exists and still passes. | **Both halves proven to execute, not assumed.** Frontend: strale-frontend CI run `32128937922`, step `Typecheck` → `tsc -b`, success; fail-before proven by the injected-error probe. Backstop: weekly-drift run [`32131992897`](https://github.com/strale-io/strale-actions/runs/32131992897) dispatched 2026-08-18 after the merge — `FTC=0`, report section shows `npm ci` installing 526 packages then `tsc -b` clean, i.e. the step did real work rather than passing vacuously. |
