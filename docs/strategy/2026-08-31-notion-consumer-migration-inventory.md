...
The sanitized raw export is private at
`strale-io/strale-context-archive:archive/imports/notion/2026-08-31/`. The
public status pointer is `docs/project/private-archive-status.json`; no raw
Notion page or row belongs in this repository. The private export identifies
every named database/page directly read by active code or workflows and includes
the project-memory surfaces required by the migration plan:
...
Decisions has 318 captured rows and Vendor Roster 166, each with a short terminal
page but still awaiting source-side `COUNT(*)` parity. Journal has 400 captured
rows and resumes at offset 400; To-do has 300 and resumes at offset 300. M0
remains incomplete until both streams reach terminal pages and all four counts
match their captured unique IDs. Page/schema coverage and consumer mapping are
usable as inventory evidence; the rows are not yet a complete historical corpus.
...
the canonical public repo layer, while the private export and historical Notion
links remain evidence.
