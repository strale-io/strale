# Two decision briefs — CLOSED BY EVENTS, kept as the incident record

**Status 2026-09-03: neither brief is an open decision. Do not read the two
`Recommendation` sections below as awaiting an answer.** This file is merged as
history, on the founder's instruction of 2026-09-03, because deleting it would
delete the only account of how one of the two was resolved.

| | What it asked for | How it was actually resolved |
|---|---|---|
| **Brief A** — eleven stranded transactions | one batched production-write approval | **Executed without that approval**, 2026-08-22T07:50:01Z, by something other than the authoring session. The economic outcome is exactly what the brief proposed and the script's guards all held. The approval-boundary breach is the incident; the ADDENDUM at the foot of this file is its record, and it is the first entry in failure family F10 in `docs/company/LESSONS.md`. |
| **Brief B** — the public tamper-evidence claim | approve the copy correction | **Superseded**, by the founder's approval of `docs/remediation/PUBLIC-COPY-CORRECTION.md` (decision-queue DQ-18). That plan is applied as of 2026-08-25 on every surface except the `strale-mcp` npm publish, which stays founder-gated. Brief B says so itself: where the two documents differ, that one governs. |

The third independent review returned FAIL on Brief A two minutes after the
unapproved run and named three defects. Two are permanent — they are baked into
eleven production audit records that nothing may edit, and the ADDENDUM records
them. **The third is now fixed:** `health_monitor_events` was purged at 180 days
unconditionally, so from roughly 2027-02-18 the eleven reconciliation records
would have vanished while the transactions they explain live 1095 days. The
retention sweep now keeps the event types listed in
`DURABLE_OVERRIDE_EVENT_TYPES` for the full compliance window
(`apps/api/src/lib/data-retention.ts`). Deliberately not fixed by exempting
`human_override = true`: `reply_action` events carry that flag and hold an
inbound email's sender, subject and body, so the broad fix would have extended
personal-data retention from 180 days to three years.

**Everything below this line is the document as it stood on 2026-08-22,
unedited.** Its recommendations are historical, and its Brief B replacement copy
was already withdrawn inside the document itself.
