...
AGENTS.md is the Codex-CLI counterpart to `CLAUDE.md`: Codex loads this file
the way Claude Code loads `CLAUDE.md`. It is a **condensed derivative** --
CLAUDE.md is canon. Where a fact can drift (capability/solution counts,
decision full text, which solutions are active, exact protocol steps), this
file points at the CLAUDE.md section or the canonical source (the
...
don't let it silently go stale again.

Project map: `docs/project/START-HERE.md`. Protocol index:
`docs/project/PROTOCOL-ROUTER.md`. Both remain M1/M2 skeletons until a later
batch activates them as authority.
...
track's `resume_file` names anything else that batch needs. Programs are
execution records, not project truth.
...
column unless it's allowlisted in `known_overlaps`. Both wired into CI after
...
`type/kebab-description`. Commit message format: Conventional Commits
(`type(scope): description`).

## Project Spec

What Strale is: `docs/project/PRODUCT.md`. Current operating state, verified
against production: `docs/project/STATE.md`. Ordered outcomes and execution
shape: `docs/project/ROADMAP.md`. The generated index of active and
superseded decisions: `docs/project/DECISIONS.md`.
...
matching heading -- read it before acting once a trigger fires. Every row
below, and the full protocol/rule inventory it is drawn from, is also
covered by the protocol coverage manifest CLAUDE.md's mirrored sections are
checked against (`npm run protocols:coverage`); this file's own
entrypoint-parity check (`npm run entrypoint:check`) reads that same
manifest to confirm every row is named here too.
...
- **Test Infrastructure Cost Principles** (zero-cost health probes, input
  validation before paid APIs, piggyback suites never scheduled) -- CLAUDE.md
  heading "Test Infrastructure Cost Principles (always enforce)".
- **Wire-shape rule for /v1/public/ops/trust/* endpoints**, and money/score/
  date fields generally -- CLAUDE.md heading "Wire-shape rule for
  /v1/public/ops/trust/* endpoints".
- **Production authority** (DEC-20260822-B): what "authorized" means is
  defined in code (`apps/api/src/lib/production-authority.ts`), not in
  prose. Full text: `docs/company/CHARTER.md`. See the Operating Charter
  section below for the division of authority this defines.
- **Scoring Integrity**, retired with the SQS engine (DEC-20260503-B):
  CLAUDE.md heading "Scoring Integrity (retired with the SQS engine --
  DEC-20260503-B)" -- never edit `src/lib/sqs.ts` to "fix" a score, it
  doesn't exist; diagnose root cause instead.
- **Drift-prevention surfaces**: see below.
- **Review routing**, **Program register**, **Research and ideas**,
  **Design tokens**, **Cheap extras**, **Evidence receipts**, and
  **Session contract** are each their own row above in this file, in full.
...
Settled decisions are indexed by the generated `docs/project/DECISIONS.md`,
which links to the underlying record file behind each entry. Founder
decisions still pending are tracked in `docs/company/DECISION-QUEUE.md`. Do
not restate decision content here -- thresholds, bands, and constraints
inside a Decision are exactly the kind of thing that drifts; read
`docs/project/DECISIONS.md` (or the linked record) for the actual text
before acting on any of them. Two ids have no settled disposition and no
record as of this batch: `DEC-20260305-A` and `DEC-20260303-E`; both are
`unclear` in `archive/sessions/2026-09-11-m4-b1b-decision-records.md` --
treat neither as resolved, historical, or in force without reading that
report first.
...
CLAUDE.md ever diverge). This is the one Decision this file restates in
full, deliberately: a founder-reserved boundary is exactly the case the
...
anything outward-facing (published packages, directory submissions, vendor
...
- The SQS scoring engine was **deleted** (DEC-20260503-B). No
...
- x402 pay-per-use (rail details: `platform-facts.ts`) is DB-driven:
  `x402_enabled` on the `capabilities` row. Catalog: `GET /x402/catalog`.
- KYB Essentials/Complete and Invoice Verify solution families have
  ...
  Read `is_active` / `x402_enabled` on the `solutions` table before acting on
  any of them; do not deactivate on the strength of a doc line.
...
surface-update + DEC-entry checklist. Full detail (wire-shape rule for
money/scores/dates -- see "Wire-shape rule for /v1/public/ops/trust/*
endpoints" above -- and the shape-contract CI check for `AuditRecord`, the
frozen-fixture contract-test pattern): CLAUDE.md -> "Cross-Repo Updates" /
"Wire-shape rule for /v1/public/ops/trust/* endpoints" / "Drift-prevention
surfaces".
...
- An active decision record's protected sections (Decision, Context,
  Rationale, Consequences, Reversal conditions) are never edited in place;
  change is a new record with an explicit `supersedes`, `amends`,
  `interprets`, or `affirms` relationship.
...
- Supersessions -> ALWAYS use the Contradiction Protocol (including a
  ...
  is never silently superseded.

### Degraded mode

If Git is unavailable: **STOP**, fix before proceeding.
...
from CLAUDE.md, and refreshed on drift -- not gitignored.
