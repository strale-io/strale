...
See .Codex/PROTOCOL.md for full criteria and protocol definitions.
...
- DEC-20260303-E: POST /v1/suggest uses Voyage AI embeddings + Codex Haiku re-ranking
...
**Trigger:** Codex detects that the session involves any of: new executor file in `src/capabilities/`, new or modified DB row in `capabilities` table, new capability slug, manifest file, seed entry, or the prompt mentions adding/creating a capability.
...
- Anything that landed without a test, and the explicit reason (e.g. "no test harness for /v1/do route-level integration; deferred per AGENTS.md test-harness exemption").
...
4. Do the work
5. **Code-review gate (before /end-session):** if any code was modified this session and `/go` was not run on it, halt and run `/go` (or escalate to Petter if a hard refusal blocks /go). Never run `/end-session` over unreviewed code. Docs / AGENTS.md / Notion-only sessions are exempt.
...
7. Do the work
8. **Code-review gate (before /end-session):** if any code was modified this session and `/go` was not run on it, halt and run `/go` (or escalate to Petter if a hard refusal blocks /go). Never run `/end-session` over unreviewed code. Docs / AGENTS.md / Notion-only sessions are exempt.
...
- Supersessions → ALWAYS use Contradiction Protocol (including AGENTS.md update)
...
For vendor switches specifically, invoke the `vendor-switch` skill (in `.Codex/skills/vendor-switch/SKILL.md`) — it codifies the full surface-update + DEC-entry checklist.
