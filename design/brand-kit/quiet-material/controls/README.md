...
The current minimum target is 44 CSS px. Ordinary navigation uses a disclosure button and native links, not ARIA menu roles. Keyboard testing and browser reflow are not full accessibility certification; screen-reader and cross-browser checks remain open. Full forms, modal dialogs, data tables and the icon library are future batches. The arrow is a decorative glyph; the caret is a simple CSS functional indicator, not a new logo or icon system.
