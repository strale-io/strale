...
Purpose: complete the CLAUDE.md Quick Session Checklist on Petter's behalf. **This command DOES write the handoff file, with front matter carrying session identity** — that's the standing preference (set 2026-04-27, carried into repo-native form at the M4 cutover). It still does not create `docs/company/DECISION-QUEUE.md` entries, `docs/decisions/records/` files, or other governance artifacts without explicit per-item approval, and it does not mutate a program register's `next_action` beyond flagging drift.
...
- Create `docs/company/DECISION-QUEUE.md` or `docs/decisions/records/` entries — those still require explicit Petter approval per CLAUDE.md governance authority thresholds.
...
b. There is no repo-native "in progress, owned by `Claude code`" per-item list to check against — the register carries one `next_action` narrative per track, not a per-item ownership column.
...
- **Contradictions with active Decisions**: per CLAUDE.md Workflow Invariants, supersessions must use the Contradiction Protocol. If this session contradicted an existing decision record without following it, flag RED.
...
- **Never write a `docs/company/DECISION-QUEUE.md` entry or a `docs/decisions/records/*.md` file on the user's behalf.** Decisions still require explicit Petter approval per CLAUDE.md governance authority thresholds. Flag if the session made a decision that needs logging.
